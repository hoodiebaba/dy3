...
class co_91_lte_nr_relation(att_xml_base):
    def create_rpc_msg(self):
        if self.usid.script.get(self.__class__.__name__, False): return
        else: self.usid.script[self.__class__.__name__] = True
        self.mos_script_list = []
        df_lte = self.usid.df_lte_rel.copy()
        df_lte = df_lte.loc[((~(df_lte.postcell.str.endswith('_L'))) & (df_lte.isdlonly == 'false'))]
        ...
        df_lte = df_lte[['postsite', 'postcell', 'dlchannelbandwidth', 'freqband']]
        df_nr['lte_ip'] = df_nr.postsite.apply(lambda x: self.usid.gnodeb.get(x).get('lte_ip', '::'))
        df_nr = df_nr[['postsite', 'postcell', 'gnbid', 'gnbidlength', 'cellid', 'nrtac', 'nrpci', 'ssbfrequency', 'ssbsubcarrierspacing',
                       'ssbperiodicity', 'ssboffset', 'ssbduration', 'freqband', 'lte_ip']]
        ...
        del df_nr, df_lte
        self.motype = 'Lrat'
        for enb in df_rel.postsite.unique():
            self.set_node_site_and_para_for_dcgk(enb)
            # self.node = enb
            # self.site = self.usid.sites.get(F'site_{self.node}')
            for gnb in df_rel.loc[(df_rel.postsite == enb)].postsite_nr.unique():
                row_gnb = df_rel.loc[((df_rel.postsite == enb) & (df_rel.postsite_nr == gnb))].iloc[0]
                mos_ldn_dict, freq_list = {}, []
                # GUtraNetwork
                mos_ldn_dict['GUtraNetwork'] = F'ENodeBFunction=1,GUtraNetwork={self.enbdata.get("GUtraNetwork", "1")}'
                freq_list += [mos_ldn_dict['GUtraNetwork']]
                gnb_nw = {'managedElementId': self.node, 'ENodeBFunction': {'eNodeBFunctionId': '1', 'GUtraNetwork': {
                    'gUtraNetworkId': self.enbdata.get('GUtraNetwork', '1')}}}
                ...
                ext_id = F'{self.usid.plmn["mcc"]}{self.usid.plmn["mnc"]}-000000{row_gnb.gnbid}'
                # ext_id = gnb
                ...
                mo_dict['ENodeBFunction']['GUtraNetwork']['ExternalGNodeBFunction'] |= {
                    'gNodeBPlmnId': self.usid.plmn, 'gNodeBId': row_gnb.gnbid, 'gNodeBIdLength': row_gnb.gnbidlength}
                ...
                mo_dict['ENodeBFunction']['GUtraNetwork']['ExternalGNodeBFunction']['TermPointToGNB'] = {
                    ...
                    'ipAddress': '0.0.0.0', 'ipv6Address': row_gnb.lte_ip}
                ...
                for row in df_rel.loc[((df_rel.postsite == enb) & (df_rel.postsite_nr == gnb))].itertuples():
                    ...
                    if mos_ldn_dict['GUtranFreqRelation'] not in freq_list:
                        ...
                        self.mo_dict[mos_ldn_dict['GUtranFreqRelation']] = {'managedElementId': self.node, 'ENodeBFunction': {
                            'eNodeBFunctionId': '1', 'EUtranCellFDD': {'eUtranCellFDDId': row.postcell,
                                                                       'GUtranFreqRelation': copy.deepcopy(mo_dict)}}}
                    # GUtranCellRelation
            ...
            #     ran_ip = F'Transport=1,Router={self.gnbdata["lte"]},InterfaceIPv6={self.gnbdata["lte_interface"]},'
            #     F'AddressIPv6={self.gnbdata["lte_add"]}'
            # else:
            #     ran_ip = F'Transport=1,Router={self.enbdata["lte"]},InterfaceIPv6={self.enbdata["lte_interface"]},'
            #     F'AddressIPv6={self.enbdata["lte_add"]}'
            if self.usid.client.software.swname <= 'ATT_22_Q3':
                ...
                self.mo_dict[F'update_ENodeBFunction=1,NodeSupport=1,ServiceDiscovery=1'] = {'managedElementId': self.node, 'NodeSupport': {
                    'nodeSupportId': '1',
                    'ServiceDiscovery': {'attributes': {'xc:operation': 'create'}, 'serviceDiscoveryId': '1',
                                         #'localAddress': ran_ip,
                                         'primaryGsds': {'host': 'localhost', 'port': '8301', 'serviceArea': 'NR'},
                                         'nodeCredential': crnode[0], 'trustCategory': crnode[1]},
                    'ServiceDiscoveryServer': {'attributes': {'xc:operation': 'create'}, 'serviceDiscoveryServerId': '1',
                                               #'localAddress': ran_ip,
            ...
            self.mo_dict[F'ENodeBFunction=1'] = {'managedElementId': self.node, 'ENodeBFunction': {
                ...
                'extendedBandN77Supported': 'true', 'ueCapabilityEnquirySteps': '1 (TWO_STEP)'}}
            # , 'intraRanIpAddressRef': lte_ip
            ...
            for row in df_rel.loc[(df_rel.postsite == enb)].groupby(['postcell']).head(1).itertuples():
                anchor_flag = False
                freqband_nr = set(df_rel.loc[((df_rel.postsite == enb) & (df_rel.postcell == row.postcell))].freqband_nr.unique())
                cell_data = {
                ...
                if row.freqband in ['14'] and len([_ for _ in nr_mb + nr_mb_plus + nr_hb if _ in freqband_nr]) > 0:
                    ...
                    anchor_flag = True
                    cell_data['additionalUpperLayerIndList'] = ['ON', 'OFF', 'OFF', 'OFF', 'OFF']
                    cell_data['endcAllowedPlmnList'] = [{"mcc": "310", "mnc": "410", "mncLength": "3"},
                    ...
                    cell_data['endcAllowedPlmnList'] = [{"mcc": "310", "mnc": "410", "mncLength": "3"},
                        ...
                        len([_ for _ in nr_mb + nr_lb if _ in freqband_nr]) > 0:
                    anchor_flag = False
                if anchor_flag:
                    ...
                    cxc4012218 = True
                    if len([_ for _ in nr_lb + nr_mb + nr_mb_plus if _ in freqband_nr]) > 0:
                        self.mos_script_list.extend([
                            F"/cm/internalmomwrite ENodeBFunction=1,EUtranCellFDD={row.postcell},ReportConfigB1GUtra=1 reportIntervalB1 2"])
