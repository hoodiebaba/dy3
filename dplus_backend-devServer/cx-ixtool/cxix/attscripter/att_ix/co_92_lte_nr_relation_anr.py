...
class co_92_lte_nr_relation_anr(att_xml_base):
    def create_rpc_msg(self):
        ...
        for enb in df_rel.postsite.unique():
            ...
            for gnb in df_rel.loc[(df_rel.postsite == enb)].postsite_nr.unique():
                ...
                # ext_id = gnb
                mos_ldn_dict['ExternalGNodeBFunction'] = F'{mos_ldn_dict["GUtraNetwork"]},ExternalGNodeBFunction={ext_id}'
                freq_list += [mos_ldn_dict['ExternalGNodeBFunction']]
                ext_nw['ENodeBFunction']['GUtraNetwork']['ExternalGNodeBFunction'] = {'externalGNodeBFunctionId': ext_id}
                mo_dict = copy.deepcopy(ext_nw)
                mo_dict['ENodeBFunction']['GUtraNetwork']['ExternalGNodeBFunction'] = \
                    self.get_mo_dict_from_moc_node_fdn_moid('ExternalGNodeBFunction', self.node, None, ext_id)
                mo_dict['ENodeBFunction']['GUtraNetwork']['ExternalGNodeBFunction'] |= {
                ...
                mo_dict['ENodeBFunction']['GUtraNetwork']['ExternalGNodeBFunction']['TermPointToGNB'] = {
                    'attributes': {'xc:operation': 'create'}, 'termPointToGNBId': ext_id, 'administrativeState': '1 (UNLOCKED)',
                    'ipAddress': '0.0.0.0', 'ipv6Address': row_gnb.lte_ip}
                self.mo_dict[F'{mos_ldn_dict["ExternalGNodeBFunction"]},TermPointToGNB={ext_id}'] = copy.deepcopy(mo_dict)
            ...
            # else:
            #     ran_ip = F'Transport=1,Router={self.enbdata["lte"]},InterfaceIPv6={self.enbdata["lte_interface"]},AddressIPv6={self.enbdata["lte_add"]}'
            ...
            self.mo_dict[F'update-ENodeBFunction=1,PmFlexCounterFilter=2[34]'] = {'managedElementId': self.node, 'ENodeBFunction': {
                'eNodeBFunctionId': '1', 'PmFlexCounterFilter': [
                    {'attributes': {'xc:operation': 'update'}, 'pmFlexCounterFilterId': '23', 'endcFilterMin': '1 (ENDC_NR_MATCHED)', 'endcFilterEnabled': 'true'},
                    {'attributes': {'xc:operation': 'update'}, 'pmFlexCounterFilterId': '24', 'endcFilterMin': '2 (ENDC_NR_ACTIVE)', 'endcFilterEnabled': 'true'}
                ]}}
