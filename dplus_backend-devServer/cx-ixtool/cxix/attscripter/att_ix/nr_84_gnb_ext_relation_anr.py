...
class nr_84_gnb_ext_relation_anr(att_xml_base):
    def create_rpc_msg(self):
        self.motype = 'GNBCUCP'
        self.nw_fdn = F'GNBCUCPFunction=1,NRNetwork={self.gnbdata["NRNetwork"]}'
        if len(self.usid.df_gnb_cell.index) == 0: return
        df_nr = self.df_gnb_cell.copy()
        df_nr = df_nr[(~((df_nr.freqband.str.contains('N260|N261|N258')) & (df_nr.carrier.str.contains('MC'))))]
        ...
        df_tr = df_tr.loc[(df_tr.postsite != self.node)]
        ...
        df_rel.drop(['flag'], axis=1, inplace=True)
        del df_tr, df_nr
        df_rel[['freq', 'relid', 'ext_id', 'ext_cell_id', 'ext_cell_mo']] = df_rel.apply(self.get_anr_type_mo_ids_for_rel,
                                                                                         axis=1, result_type='expand')

        me_dict = {'managedElementId': self.node, 'GNBCUCPFunction': {'gNBCUCPFunctionId': '1'}}
        mo_list = []
        for row in df_rel.itertuples():
            mos_ldn_dict = {}
            # NRFrequency
            ...
            if mos_ldn_dict['NRFrequency'] in mo_list: pass
                ...
                mo_dict['GNBCUCPFunction']['NRNetwork'] = {'nRNetworkId': self.gnbdata['NRNetwork'], 'NRFrequency': tmp_dict}
            ...
            if row.ext_id:
                ...
                if mos_ldn_dict['ExternalGNBCUCPFunction'] not in mo_list:
                    ...
                    mo_dict = copy.deepcopy(me_dict)
                    mo_dict['GNBCUCPFunction']['NRNetwork'] = {'nRNetworkId': self.gnbdata['NRNetwork'], 'ExternalGNBCUCPFunction': tmp_dict}
                ...
                if mos_ldn_dict['ExternalNRCellCU'] in mo_list: pass
                    ...
                    tmp_dict |= {'cellLocalId': row.cellid, 'nRPCI': row.nrpci, 'nRTAC': row.nrtac, 'nRFrequencyRef': mos_ldn_dict.get('NRFrequency')}
                    mo_dict = copy.deepcopy(me_dict)
                    mo_dict['GNBCUCPFunction']['NRNetwork'] = {
                        'nRNetworkId': self.gnbdata['NRNetwork'],
                        'ExternalGNBCUCPFunction': {'externalGNBCUCPFunctionId': row.ext_id, 'ExternalNRCellCU': tmp_dict}}
