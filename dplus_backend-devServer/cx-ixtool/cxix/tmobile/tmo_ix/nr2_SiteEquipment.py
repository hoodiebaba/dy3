...
class nr2_SiteEquipment(tmo_xml_base):
    def initialize_var(self):
        if self.gnbdata.get('equ_change') and not self.gnbdata.get('mmbb'):
            self.gnbdata["SE_File"] = F'{self.__class__.__name__}_{self.node}.xml'
            self.relative_path = [self.node, self.gnbdata["SE_File"]]
    ...
    def create_rpc_msg(self):
        ...
        if self.gnbdata['bbtype'] == '5216': ri_no = 6
        elif self.gnbdata['bbtype'] == '6648': ri_no = 12
        elif self.gnbdata['bbtype'] == '6651': ri_no = 12
        elif self.gnbdata['bbtype'] == '6630': ri_no = 15
        else: ri_no = 15
        ...
        tmp_dict = {
            ...
            'Equipment': {
                ...
                'EcBus': {'ecBusId': '1', 'ecBusConnectorRef': F'ManagedElement=1,Equipment=1,FieldReplaceableUnit={self.gnbdata["bbuid"]}',
                          'equipmentSupportFunctionRef': 'ManagedElement=1,EquipmentSupportFunction=1'},
                'FieldReplaceableUnit': {'fieldReplaceableUnitId': self.gnbdata['bbuid'], 'administrativeState': 'UNLOCKED',
            ...
            'NodeSupport': {
                ...
                'MpClusterHandling': {'mpClusterHandlingId': '1',
                                      'primaryCoreRef': F'ManagedElement=1,Equipment=1,FieldReplaceableUnit={self.gnbdata["bbuid"]}'},
        ...
        if self.gnbdata['bbtype'] == '6648': del tmp_dict['Equipment']['FieldReplaceableUnit']['EcPort']
        elif self.gnbdata['bbtype'] == '6651': del tmp_dict['Equipment']['FieldReplaceableUnit']['EcPort']
        me_mo = self.mo_add_form_dict_xml(tmp_dict, 'ManagedElement', change_me=False)
        config.appendChild(me_mo)

        # SUP - Start
        if '5216' in self.gnbdata['bbtype']:
            tmp_dict = {'fieldReplaceableUnitId': 'SUP-1', 'administrativeState': 'UNLOCKED',
